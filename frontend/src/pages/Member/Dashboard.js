    ...
    <DashboardLayout role="member">
      <div className="space-y-6 animate-fade-in" data-testid="member-dashboard">
        <div>
          ...
          <p className="text-zinc-500">Here's your fitness journey overview</p>
        </div>
              ...
              {membership ? (
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4 pt-4">
                    <div>
                      <p className="text-xs uppercase text-zinc-500">Start Date</p>
                      <p className="text-white font-medium">{formatDate(membership.start_date)}</p>
                    </div>
                    <div>
                      <p className="text-xs uppercase text-zinc-500">Expiry Date</p>
        ...
        <Card className="glass-card">
          ...
          <CardContent className="p-6">
            {announcements.length === 0 ? (
              <p className="text-zinc-500 text-center py-4">No announcements</p>
            ) : (
              <div className="space-y-4">
                {announcements.map((ann) => (
                  <div key={ann.id} className="p-4 bg-zinc-900/50 rounded-lg border border-zinc-800">
                    <h3 className="font-semibold text-white">{ann.title}</h3>
                    <p className="text-zinc-400 text-sm mt-1">{ann.content}</p>
                    <p className="text-zinc-600 text-xs mt-2">{formatDate(ann.created_at)}</p>
                  </div>
                ))}
              </div>
            )}
    ...
    <DashboardLayout role="member">
      <div className="space-y-6 animate-fade-in" data-testid="member-plans">
        <div>
          ...
          <p className="text-zinc-500">Choose a plan that fits your fitness goals</p>
        </div>
